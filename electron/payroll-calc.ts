 ...
 *
 * ⚠ This file is kept in sync with `src/shared/payroll/calc.ts`.
 * The renderer cannot import from the `electron/` tree and the electron
 * tsconfig `rootDir` refuses to pull files from outside `electron/`, so we
 * duplicate the pure module here. Any change MUST be applied to both files.
 *
 * NOTE ON ACCURACY
 * ----------------
 ...
 *
 * Rate sources (as of the 2026 April snapshot bundled with v0.2.0):
