...
function derivePerms(role: Role): SessionUser['perms'] {
  const executive = role === 'CEO' || role === 'CTO';
  const leadership = executive || role === 'OPS_MANAGER';
  const ta = role === 'TA';
  // 정규직(= 조교가 아닌 사람). TA가 업로드한 파싱 엑셀을 보고 외부 프로그램으로
  // 수행평가를 생성하는 "소비자" 레인.
  const regularStaff = !ta;
  return {
    canReviewQA1: regularStaff,
    canReviewQAFinal: regularStaff,
    // TA도 파싱 업로드는 할 수 있다. 단, 실제 assignments 생성은 정규직만.
    canParseAssignments: true,
    canManagePeople: !ta && (role === 'HR_ADMIN' || executive),
    canApprove: !ta && (leadership || role === 'HR_ADMIN'),
    isLeadership: leadership,
    canReviewParsedExcel: regularStaff,
